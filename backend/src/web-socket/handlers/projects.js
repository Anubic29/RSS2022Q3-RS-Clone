...
const projectProperties = ['title', 'description', 'color', 'key', 'pathImage'];
...
async function updateProject(data, objDuct) {
  ...
  if (author) {
    if (!project.team.includes(author)) throw new Error(`There is no user ${author} in team`);

    const admin = project.author;
    project.author = author;
    project.team.splice(project.team.findIndex((data) => data === author), 1, admin);
  }
...
async function removeProject(data, objDuct) {
  ...
  const socketsArrOnlyTeam = Object.keys(objDuct.all)
    .filter((key) => project.team.includes(key))
    .filter((key) => !!objDuct.all[key])
    .map((key) => objDuct.all[key]);
