...
async function joinHandler(socket, userId, projectId) {
  ...
  if (!project || project.author !== userId && !project.team.includes(userId)) {
  ...
  const filteredUsers = users.filter((user) => project.team.includes(user._id.toString()) || user._id.toString() === project.author)
  const userAnswer = JSON.stringify({type: 'team', method: 'get', data: filteredUsers});

  const user = await User.findOne({ _id: userId });
  if (!user) throw new Error('Not found user');

  if (user.recentProjects.includes(projectId)) {
    user.recentProjects.splice(user.recentProjects.findIndex((data) => data === projectId), 1);
