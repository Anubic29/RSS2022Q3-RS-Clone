...
async function addUser(data, objDuct) {
  ...
  if (!project.team.includes(user._id.toString())) project.team.push(user._id);

  await project.save();

  const answerAdded = JSON.stringify({type: 'team', method: 'get', data: [user]});
  ...
  const socketsArrOther = Object.keys(objDuct.all)
    .filter((key) => [project.author, ...project.team].includes(key) && key !== objDuct.current.id)
    .filter((key) => !!objDuct.all[key])
    .map((key) => objDuct.all[key])
    .map((array) => array.filter((client) => client.projectEnv === project._id.toString()));
...
async function removeUser(data, objDuct) {
  ...
  const idx = project.team.indexOf(data._id);
  ...
  const socketsArrOther = Object.keys(objDuct.all)
    .filter((key) => [project.author, ...project.team].includes(key) && key !== data._id && key !== objDuct.current.id)
    .filter((key) => !!objDuct.all[key])
    .map((key) => objDuct.all[key])
    .map((array) => array.filter((client) => client.projectEnv === project._id.toString())); 
  socketsArrOther.forEach((array) => {
