...
function TaskPopup() {
  ...
  const isNoted = useMemo(
    () => notedList.some((data) => data.id === params.taskId),
  ...
  const options = useMemo(
    () => columnList.map((data) => ({ text: data.title, value: data._id })),
    [columnList]
  );
        ...
        {task ? (
          <>
            <div className={styles['popup-header']}>
              <Link to={`/projects/${params.id}/board/selected-task/${task?._id}`}>
              ...
              <input
                ...
                readOnly
              />
              <button className={styles['copy-button']} onClick={clipboard.copy}>
                <BsLink45Deg className={styles['copy-icon']} />
              </button>
            ...
            <div className={styles['popup-content']}>
              ...
              <div className={styles['aside-content']}>
                <div className={styles['info-line']}>
                  <div className={styles['select-panel']}>
                    {options.length && (
                      <SelectPanel
                        ...
                        selectedItem={options.findIndex(
                          (option) => option.value === task?.columnId
                        )}
                  ...
                  {isNoted && <MdStar className={styles['noted-icon']} />}
                </div>
                <DetailsBlock
                  taskId={`${task?._id}`}
