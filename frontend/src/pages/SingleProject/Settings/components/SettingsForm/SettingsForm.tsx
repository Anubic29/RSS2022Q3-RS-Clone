...
import { Button, Input, Label, Preloader, ProjectAvatar } from '../../../../../components';
...
function SettingsForm() {
  ...
  const isBtnDisabled = useMemo(() => {
    ...
    if (!currentProject) return true;
      ...
      return (
        ...
        (!values.projectImage || currentProject.pathImage === values.projectImage) &&
        (!values.projectColor || currentProject.color === values.projectColor)
      );
  ...
  const showPopupHandler = useCallback(
    (pathImage: string) => {
      dispatch(
        setOverlayContent(
          <PopupProjectBadges
            setImageSrc={(value) => setValues((prev) => ({ ...prev, projectImage: value }))}
            setImageBg={(value) => setValues((prev) => ({ ...prev, projectColor: value }))}
            imageSrc={values.projectImage ?? pathImage}
          />
        )
      );
    },
  ...
  const onSubmitHandler = useCallback(() => {
    ...
    if (currentProject) {
      ...
      if (values.projectImage) updatedProject['pathImage'] = values.projectImage;
      if (values.projectColor) updatedProject['color'] = values.projectColor;
      ...
      {currentProject && (
        <div className={styles['form']}>
          <div className={styles['project-avatar']}>
            {values.projectImage && <div className={styles['changed']}>CHANGED</div>}
            <ProjectAvatar
              source={values.projectImage ?? currentProject.pathImage}
              typeSize="large"
            />
            <Button onClick={() => showPopupHandler(currentProject.pathImage)}>
              Change avatar
            </Button>
          </div>
